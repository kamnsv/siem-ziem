...
from .ndr import NDRCALL, NDRSTRUCT, NDRPOINTER, NDRUniConformantArray, NDRTLSTRUCT, UNKNOWNDATA
from .dtypes import LPWSTR, ULONGLONG, HRESULT, GUID, USHORT, WSTR, DWORD, LPLONG, LONG, PGUID, ULONG, \
    UUID, WIDESTR, NULL
from . import hresult_errors, LOG
from .uuid import string_to_bin, uuidtup_to_bin, generate
from .rpcrt import TypeSerialization1, RPC_C_AUTHN_LEVEL_PKT_INTEGRITY, RPC_C_AUTHN_LEVEL_NONE, \
...
class DCOMConnection:
    ...
    async def initConnection(self):
        ...
        await self.__portmap.connect()
        DCOMConnection.PORTMAPS[self.__target] = self.__portmap

    async def CoCreateInstanceEx(self, clsid, iid):
        scm = IRemoteSCMActivator(self.__portmap)
        iInterface = None
        try:
            iInterface = await scm.RemoteCreateInstance(clsid, iid)
        except Exception as e:
...
class INTERFACE:
        ...
        if (self.__target in INTERFACE.CONNECTIONS) is True:
            if currentThread().getName() in INTERFACE.CONNECTIONS[self.__target] and \
                ...
                if currentBinding == iid:
                    ...
                    newDce = await dce.alter_ctx(iid)
    ...
    async def request(self, req, iid = None, uuid = None):
        ...
        await self.connect(iid)
        dce = self.get_dce_rpc()
        try:
            resp = await dce.request(req, uuid)
        except Exception as e:
            logging.error('dcom.request err: ' + repr(e).replace('\n', '\r'))
...
class IRemUnknown(INTERFACE):
    ...
    async def RemRelease(self):
        ...
        element['cPublicRefs'] = 1
        request['InterfaceRefs'].append(element)
        resp = await self.request(request, IID_IRemUnknown, self.get_ipidRemUnknown())
...
class IRemoteSCMActivator:
    ...
    async def RemoteCreateInstance(self, clsId, iid):
        # Only supports one interface at a time
        await self.__portmap.bind(IID_IRemoteSCMActivator)
        ORPCthis = ORPCTHIS()
        ORPCthis['cid'] = generate()
        ORPCthis['extensions'] = NULL
        ORPCthis['flags'] = 1
        ...
        request['pActProperties']['ulCntData'] = len(objrefcustom.getData())
        request['pActProperties']['abData'] = list(objrefcustom.getData())
        resp = await self.__portmap.request(request)
