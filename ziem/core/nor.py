...
class Normaleizer:
    ...
    def rule_mswindows_not(self, alert):
        ...
        if code in self.filter:
            ...
            for field, value in self.filter[code].items():
                if field == 'regex':
                    for regex_field, regex_value in value.items():
                        try: 
                            # mapping 
                            raw_value = strings[int(regex_value)]
                            if raw_value in mapping:
                                raw_value = mapping[raw_value]
                            event[regex_field] = raw_value
                        except: continue
                else:
                    event[field] = value
            if 'process' in event:
            ...
            if alert.get('alr_sub'): event['system'] = alert.get('alr_sub')
            event['raw'] = {
                'raw' : alert['alr_raw'],
                'code': alert['alr_code'],
                'log': alert['alr_log'],
            ...
            #if len(mapping): event['raw']['mapping'] = mapping
    ...
    def rule_syslog(self, alert):
        ...
        for search_string, event_data in self.filter.items():
            #if search_string in alert['alr_raw']:
            if search(search_string, alert['alr_raw']):
                event['node'] = alert['alr_node']
                event['ip'] = alert['alr_ip']
                for field, value in event_data.items():
                    if field == 'regex':
