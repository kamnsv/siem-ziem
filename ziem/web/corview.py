...
def deeprule_add():
    ...
    for event in form.events:
        ...
        for incfilter in event.incfilter:
            ...
            incfilter.book.choices = get_choice('bks')
        for excfilter in event.excfilter:
            excfilter.field.choices = get_choice('field')
            excfilter.book.choices = get_choice('bks')
    if form.validate_on_submit():
        data = form.data
        data['pubdate'] = datetime.now().strftime("%Y-%m-%d")
...
def deeprule_edit(id):
    ...
    if form.validate_on_submit():
        ...
        del data['csrf_token']
        for event in data['events']:
            event['incfilter'] = [ x for x in event['incfilter'] if x['value'] or x['book'] ]
            event['excfilter'] = [ x for x in event['excfilter'] if x['value'] or x['book'] ]
        """
        for k, v in data.items():
            if k in doc:
                if v != doc[k]:
