...
from .optform import OptForm, JsonForm, SearchForm, BksForm
from .bksview import str_to_date
...
def opt_edit(option, id):
    ...
    if 'opt_bks' != option:
        ...
        if 'scheduled' in doc: doc['scheduled'] = str_to_date(doc.get('scheduled'))
        if 'updated' in doc: doc['updated'] = str_to_date(doc.get('updated'))
        form = BksForm(data=doc)  
    ...
    if form.validate_on_submit():
        ...
        if 'opt_bks' == option and data['token'] == '':
            del data['token']
    ...
    if 'opt_bks' == option:
        return render_template('opt/bks_edit.html', form=form, option=option)
...
def get_systems():
    ...
    sub = {}
    for x in col.find():
        if get_col('log_rule').find({'sub': {'$eq': x['name']}}).count():
            sub.update({ x['name']:x['desc']  })
    return sub
...
def menu_systems():
    ...
    for k, v in systems.items():
        #li ms-4
        menu += f'''
               <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small rule_sub">
                  <li class="">
                    <a class="nav-link rounded p-0" 
                    href="/log/rule_{k}/" 
                    data-menu="rule_{k}" 
                    aria-current="/log/rule_{k}/">
                      {v}
                    </a>
                  </li>
                </ul>
