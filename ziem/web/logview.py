...
def rule():
    form = SearchForm()
    form.selected_type.choices = get_choice('type')
    return render_template('log/rule.html', form=form)

@bp.route('/log/rule_<string:sub>/')
@login_required
def rule_sub(sub):
    form = SearchForm()
    form.selected_type.choices = get_choice('type')
    return render_template('log/rule_sub.html', form=form, sub=sub)

@bp.route('/log/templates/')
@login_required
def templates():
    form = SearchForm()
    form.selected_type.choices = get_choice('type')
    return render_template('log/rule_tmp.html', form=form)
...
@bp.route('/log/get_rule/<string:sub>', methods=('GET', 'POST'))
@login_required
def get_rule_sub(sub):
    return get_log_rule(sub=sub)

@bp.route('/log/get_tmp', methods=('GET', 'POST'))
@login_required
def get_tmp():
    return get_log_rule(active=False)


def get_log_rule(active=True, sub=None):
    
    find = {'obj':'main', 'active': active}
    
    type_log = request.args.get("type")
    if type_log:
        find['type'] = type_log
    ...
    data = [ x for x in col.find(find).sort("name") ]
...
def rule_add():
    ...
    form.sub.choices = get_choice('sub')
    form.type.choices = get_choice('types')
...
def rule_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if form.new_active.data:
            doc['name'] = doc['name'] + '_new'
            doc['active'] = True
            del doc['_id']
            col.insert_one(doc)
        else:    
...
@bp.route('/log/templates/<string:id>/copy')
...
@bp.route('/log/templates/<string:id>/del')
