...
function sortTableByColumn(e, tableId, columnNumber) {
  ...
  columnNumber = 0;
  for (th of e.parentNode.querySelectorAll('th')) {
      if (e == th) break;
      columnNumber++;
  }
  ...
  for (th of tableElement.querySelectorAll('th')) 
  ...
  e.innerHTML = `
    <div class="border-top d-flex align-items-center justify-content-end p-2">
      ...
      <select 
        ...
        style="width:60px"
        id="page_max" 
        onchange="searchRow(this.value)" value="25">
...
function searchRow() {
    ...
    for (var tr of trs) {
        var tds = tr.querySelectorAll("tbody td");
        var text = Array.from(tds).map(e => e.textContent.trim()).join(', ');
    ...
    // Set paginator
    var per_page = document.getElementById("page_max").value;
    var items = document.querySelectorAll("tbody tr.matched");
    document.getElementById("table-count").textContent=items.length;
    var page_count = Math.floor(items.length/per_page) + (items.length%per_page > 0) 
    document.getElementById("pages").dataset.page=page_count
...
function switchPage(action) {
  ...
  var page_max = parseInt(document.getElementById("pages").dataset.page);
  var page_current; 
  if ('next' == action){
    page_prev.classList.remove('disabled');
    page_current = page_next.dataset.page;
    page_next.dataset.page++;
    page_prev.dataset.page++;
  }
  else if ('prev' == action){
    page_next.classList.remove('disabled');
    page_current = page_prev.dataset.page;
    page_next.dataset.page--;
    page_prev.dataset.page--;
  }
  else page_current = 1;
  ...
  else if (page_next.dataset.page > page_max) {
    page_next.classList.add('disabled');
  }

  document.getElementById("page").textContent=page_current; 

  var items = document.querySelectorAll('tbody tr');
  for (i of items) i.classList.add("d-none");
    
  var matched = document.querySelectorAll('.matched');  
  var a = (page_current - 1) * per_page;
  var b = page_current * per_page;
  for (i = a; i < b; i++)
      if (i in matched)
          matched[i].classList.remove("d-none");
  ...
  for (i = 1; i < tr.length; i++) {
    if (tr[i].style.display != "none"){
      ...
      if ( check_box.checked == true){
        table.deleteRow(i);
        i--;
        console.log(url + check_box.value + "/del");
...
function rawToYaml(){
    let raws = document.querySelectorAll('td[data-type="Сырое событие"');
    for (raw of raws){
        data = raw.textContent.trim();
        //console.log(data);
        try{
        ...
        //console.log(data)
        raw.innerHTML = format_code(data);
    }
...
// toggle load btn
function toggle_load_btn(e, label){
    if (undefined == label) label =  'Загрузка...'
    ...
    if (e.classList.contains('disabled'))
        e.innerHTML=`<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> ` + label;
    else
...
function set_menu_active(){
    ...
    if (path) {
      var obj = document.querySelector('[aria-current="' + path + '"]');
      if (null == obj) return;
      obj.classList.add('active');
      obj.parentNode.classList.add('active');
      obj.parentNode.parentNode.parentNode.classList.add('show');
      const menu = document.getElementById(obj.dataset.menu);
      menu.classList.add('active');
    };
}

function hidden_panel(){
    console.log();
    if (location.pathname == '/main/status/minute')
        document.querySelector("#extra_panel").style.visibility = 'hidden';
}
