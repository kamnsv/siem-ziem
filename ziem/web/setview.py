...
from .setform import SettingForm, JsonForm, SearchForm, MapsearchForm, SenderForm

from ziem.post import Prots, Sender
...
def setting():
    col = get_col('set')
    doc = col.find()
    form = SettingForm(data={'senders': doc})
    return render_template('set/setting.html', form=form, data=form.data)

@bp.route('/set/sender/test/', methods=['POST'])
@login_required
def prot_test():
    opt = request.json
    sender = None
    try:
        ...
        logging.debug(json.dumps(opt))
        return 'Параметры сендера не валидны', 400
    ...
    count = 5 # кол-во тестовых сообщений
    
    for col in ('alerts','events','incs'):
        if not getattr(sender, col).get('on'): continue
        
        # формирование тестовых данных
        data = []
        for i in range(count):
            data.append({'test': sender.name, 
                         'collection': col, 
                         'id': i+1, 
        ...
        # отправка данных
        code = asyncio.run(getattr(prot, f'send_{sender.prot}')(col, data, sender))

        # интерпритация результата    
        if -1 == code:
            res[col] = f'Успех теста {col}'
        elif -2 == code:
            res[col] = f'Ошибка соединения для {col}'
        else:
            res[col] = f'Передача {col} прервана на {code+1} сообщении из {count}' 
          
    return '</br>'.join(list(res.values())), 200
...
def post_setting():
    ...
    if form.validate_on_submit():
        ...
        for s in data['senders']:
            if s['name'] in names:
                return 'Не уникальные названия сендеров', 400
            names.append(s['name'])
        col.drop();    
        for s in data['senders']:    
            col.insert_one(s);
    ...
    with tarfile.open(tar_file, 'w:bz2') as tar:
        for k,v in cols.items():
            ...
            with open(file, "w") as f:
                json.dump(data_col, f, indent=4, ensure_ascii=False, default=str)
            tar.add(file, k + '.json')
            data[k] = data_col
...
def get_colllection():
    cols = {
        ...
        'opt_bks': get_col('opt_bks'),
        'opt_sub': get_col('opt_sub'),
        'set': get_col('set'),
    }    
    db = get_rawdb()
    col = db['opt_bks']
