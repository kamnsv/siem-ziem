...
from . import ntlm, LOG
from .structure import Structure,pack,unpack
from .uuid import uuidtup_to_bin, generate, stringver_to_bin, bin_to_uuidtup
from .dtypes import UCHAR, ULONG, USHORT
from .ndr import NDRSTRUCT
from . import hresult_errors
...
class DCERPC:
    ...
    async def call(self, function, body, uuid=None):
        if hasattr(body, 'getData'):
            return await self.send(DCERPC_RawCall(function, body.getData(), uuid))
        else:
            return await self.send(DCERPC_RawCall(function, body, uuid))
...
class DCERPC_v5(DCERPC):
        ...
        if should_fragment:
            ...
            while 1:
                ...
                if offset >= len(packet):
                    ...
                    data['flags'] &= (~PFC_LAST_FRAG)
                data['pduData'] = toSend
                await self._transport_send(data, forceWriteAndx = 1, forceRecv =data['flags'] & PFC_LAST_FRAG)
        else:
    ...
    async def recv(self):
        ...
        while not finished:
            ...
            #print('RECV frag len:', response_header['frag_len'])
            while len(response_data) < response_header['frag_len']:
               #print('RECV:start:',len(response_data))
               response_data += await self._transport.recv(forceRecv, count=(response_header['frag_len']-len(response_data)))
               #print('RECV:end:',len(response_data))
    ...
    async def alter_ctx(self, newUID, bogus_binds = 0):
        ...
        await answer.bind(newUID, alter = 1, bogus_binds = bogus_binds, transfer_syntax = bin_to_uuidtup(self.transfer_syntax))
        return answer
